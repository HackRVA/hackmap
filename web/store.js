...
export function generateContainersCSV() {
  const rows = ['id,x,y,width,height,label,rotation,wikiPage,description,imageUrl'];
  containers.forEach((container) => {
    const rect = container.rect;
    rows.push(`${rect.id},${rect.x},${rect.y},${rect.width},${rect.height},${rect.label},${rect.rotation},${container.wikiPage},${container.description},${container.imageUrl}`);
  });
  return rows.join('\n');
}
...
export function generateItemsCSV() {
  ...
  items.forEach((item) => {
    rows.push(`${item.name},${item.containerId},${item.wikiPage},${item.description},${item.imageUrl}`);
  });
  return rows.join('\n');
}
...
if (!useServer) {
  ...
  const container2 = addContainer(1165.6989130434783, 118.90108695652177, 200, 150, "Storage Cabinet");
  const container3 = addContainer(990.7923913043478, 536.6184782608696, 100, 100, "Itembox");
  const container4 = addContainer(1962.845652173913, 144.15652173913043, 250, 200, "Shelving Unit");
  const container5 = addContainer(2023.05, 676.2521739130434, 300, 150, "Workstation");
